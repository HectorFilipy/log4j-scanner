#!/usr/bin/env python3
#
#  Arquivo de scan para encontrar Log4j arquivos que estão vulneraveis para Log4Shell (CVE-2021-44228)
...
|__||_____|___  |   |__||  |      |__| |__||__|__|_____||_____|__|
          |_____|      |___| v{__version__}
"""

# Opcionalmente, importe o colorama para habilitar a saída colorida para Windows.
try:
    import colorama
...
# Nomes de arquivos a serem encontrados e hash MD5 (também recursivamente em JAR_EXTENSIONS)
# Atualmente, procuramos apenas JndiManager.class
FILENAMES = [
    p.lower()
    for p in [
        "JndiManager.class",
...
MD5_BAD = {
    ...
    # 2.15.0 vulneraveis a ataque de negação de serviço (source: https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-45046)
...
def md5_digest(fobj):
    """Calcule o MD5 de um objeto de arquivo."""
    d = hashlib.md5()
...
def iter_scandir(path, stats=None):
    """
    Rende todos os arquivos correspondentes a JAR_EXTENSIONS ou FILENAMES recursivamente no caminho
    """
    p = Path(path)
    if p.is_file():
        if stats:
...
def scantree(path, stats=None):
    """Rende recursivamente objetos DirEntry para determinado diretório."""
    try:
...
def iter_jarfile(fobj, parents=None, stats=None):
    """
    Rendimentos (zfile, zinfo, zpath, pais) para cada arquivo em zipfile que corresponda a `FILENAMES` ou` JAR_EXTENSIONS` (recursivamente)
    """
    parents = parents or []
    try:
        with zipfile.ZipFile(fobj) as zfile:
            for zinfo in zfile.infolist():
                ...
                if zpath.name.lower() in FILENAMES:
                    ...
                    try:
                        # Teste se podemos abrir o zfobj sem erros, fallback para BytesIO caso contrário
...
def check_vulnerable(fobj, path_chain, stats, has_jndilookup=True):
    """
    Teste se fobj corresponde a algum dos hashes MD5 conhecidos ou ruins.
    Também imprime mensagem se fobj for vulnerável ou bom ou desconhecido.

    se `has_jndilookup` for False, significa que` lookup / JndiLookup.class` não foi encontrado e pode
    indicar que foi corrigido de acordo com https://logging.apache.org/log4j/2.x/security.html usando:
...
def print_summary(stats):
    print("\nResumo:")
    print(f" Processado {stats['files']} arquivos e {stats['directories']} diretorios")
    print(f" Scaneado {stats['scanned']} arquivos")
    if stats["vulnerable"]:
        print("  Encontrado {} arquivos vulneraveis".format(stats["vulnerable"]))
    if stats["good"]:
        print("  Encontrado {} arquivos bons".format(stats["good"]))
    if stats["patched"]:
        print("  Encontrado {} arquivos corrigidos".format(stats["patched"]))
    if stats["unknown"]:
        print("  Encontrado {} arquivos desconhecidos".format(stats["unknown"]))


def main():
    parser = argparse.ArgumentParser(
        description=f"%(prog)s v{__version__} - Encontrar vulneravel log4j2 no sistema de arquivos (Log4Shell CVE-2021-4428, CVE-2021-45046)",
        epilog="Os arquivos são verificados recursivamente, tanto em disco quanto em Java Archive Files",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["/"],
        help="Diretório ou arquivo(s) para verificar (recursivamente)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
        default=0,
        help="Saida detalhada (-v is info, -vv is debug)",
    )
    parser.add_argument(
        "-n", "--no-color", action="store_true", help="desativar a saída de cores"
    )
    parser.add_argument(
        "-q",
        "--quiet",
        action="store_true",
        help="fique mais quieto, desativa banner e resumo.",
    )
    parser.add_argument("-b", "--no-banner", action="store_true", help="desativar banner")
    parser.add_argument(
    ...
    if args.verbose == 1:
        ...
        log.info("info logging ativado")
    elif args.verbose >= 2:
        log.setLevel(logging.DEBUG)
        log.debug("debug logging ativado")
    ...
    for directory in args.path:
        now = datetime.datetime.utcnow().replace(microsecond=0)
        if not args.quiet:
            print(f"[{now}] {hostname} Escaneando: {directory}")
        for p in iter_scandir(directory, stats=stats):
            if p.name.lower() in FILENAMES:
                stats["scanned"] += 1
                log.info(f"Arquivo encontrado: {p}")
                with p.open("rb") as fobj:
                    # Se encontrarmos JndiManager, também verificamos se JndiLookup.class existe
                    has_lookup = True
            ...
            if p.suffix.lower() in JAR_EXTENSIONS:
                try:
                    log.info(f"Arquivo jar encontrado: {p}")
                    stats["scanned"] += 1
                    for (zinfo, zfile, zpath, parents) in iter_jarfile(
                        p.resolve().open("rb"), parents=[p.resolve()]
                    ):
                        log.info(f"zfile encontrado: {zinfo} ({parents}")
                        with zfile.open(zinfo.filename) as zf:
                            # Se encontrarmos JndiManager.class, também verificamos se JndiLookup.class existe
                            has_lookup = True
    ...
    now = datetime.datetime.utcnow().replace(microsecond=0)
    if not args.quiet:
        print(f"[{now}] {hostname} Scan finalizado, tempo decorrido: {elapsed:.2f} segundos")
        print_summary(stats)
